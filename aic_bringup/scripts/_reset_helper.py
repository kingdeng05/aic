...
def _publish_home_joint_commands(duration_s: float) -> None:
    """Publish home joint commands at 50 Hz for `duration_s` seconds."""
    joint_cmd = (
...
def robot_home_init():
    """Reset joints to home and leave the controller actively holding home in
    joint mode. Does NOT switch back to Cartesian mode — pair with
    `robot_home_finalize()` after any work that depends on the gripper being
    at home (e.g. cable spawn/attach)."""
    ...
    _publish_home_joint_commands(5.0)
    print("robot_home_init complete (controller still in joint mode)")
    return True


def robot_home_finalize():
    """Send a final reaffirming joint-home burst, then switch the controller
    back to Cartesian mode and tare the FT sensor."""
    print("Reaffirming home joint commands for 1 second...")
    _publish_home_joint_commands(1.0)
    ...
    print("robot_home_finalize complete")
    return True


def home():
    """Backward-compatible combined home: init + finalize."""
    return robot_home_init() and robot_home_finalize()
...
if __name__ == "__main__":
    ...
    elif cmd == "robot_home_init":
        ok = robot_home_init()
        sys.exit(0 if ok else 1)
    elif cmd == "robot_home_finalize":
        ok = robot_home_finalize()
        sys.exit(0 if ok else 1)
